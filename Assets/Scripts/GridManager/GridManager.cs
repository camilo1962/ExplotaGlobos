...
public class GridManager : MonoBehaviour
{
    ...
    [SerializeField] float duracion, currentTime;
    ...
    IEnumerator TimeIEn()
    {
        while(currentTime >= 0)
        {
            timeImage.fillAmount = Mathf.InverseLerp(0, duracion, currentTime);
            timeText.text = currentTime.ToString();
        ...
        finalScore.text = pscore.ToString();
        if(pscore > PlayerPrefs.GetInt("record"))
        {
            PlayerPrefs.SetInt("record", pscore);
            recordText.text = PlayerPrefs.GetInt("record").ToString();

        }
        finalRecord.text = PlayerPrefs.GetInt("record").ToString();
        soundManager = GameObject.FindGameObjectWithTag("SoundManager").GetComponent<SoundManager>();
        soundManager.StopClip(soundManager.musicClip1);
